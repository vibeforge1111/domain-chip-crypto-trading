...
class MetaAgent:
    ...
    def __init__(
        ...
        # PT-validated agent IDs for breeding priority (lazy-loaded)
        self._pt_validated_ids: set[str] = set()
        self._pt_state_gen: int = -1
    ...
    def generate_agents(
        ...
        # Merge dead-end patterns from Spark insights (cross-session memory)
        try:
            from . import spark_bridge
            spark_dead_ends = spark_bridge.load_dead_end_patterns()
            for de in spark_dead_ends:
                pattern = de.get("pattern", "")
                if pattern:
                    dead_ends.add(pattern)
        except Exception:
            pass  # Spark unavailable -- no problem
    ...
    def _feature_guided(
        ...
        # Load Spark guard effectiveness data for informed selection
        _spark_top_guards = []
        try:
            from . import spark_bridge
            guard_eff = spark_bridge.load_guard_effectiveness()
            # Guards with delta > 0.03 are Spark-validated winners
            _spark_top_guards = [
                g for g, delta in guard_eff.items()
                if delta > 0.03 and g in GUARD_MUTATIONS
            ]
        except Exception:
            pass

        if random.random() < params.guard_add_prob:
            if _spark_top_guards and random.random() < 0.40:
                # 40% chance: pick from Spark-validated effective guards
                guard = random.choice(_spark_top_guards)
    ...
    def _load_pt_state(self, generation: int):
        """Lazy-load PT-validated agent IDs from pt_state.json.

        Called once per generation. Gives PT-validated agents a slight
        breeding advantage so evolution preferentially reproduces from
        agents that have been confirmed in paper trading.
        """
        if generation == self._pt_state_gen:
            return  # already loaded this generation
        pt_path = Path(__file__).resolve().parent.parent / "archive" / "pt_state.json"
        if pt_path.exists():
            try:
                state = json.loads(pt_path.read_text(encoding="utf-8"))
                self._pt_validated_ids = {
                    aid
                    for aid, data in state.get("agent_pt_index", {}).items()
                    if data.get("status") in ("validated", "live_ready")
                }
            except Exception:
                pass
        self._pt_state_gen = generation
    ...
    def _select_parent(self, params: MetaStrategyParams) -> Agent | None:
        ...
        # Load Spark-validated guard names for fitness bonus
        _spark_effective_guards = set()
        try:
            from . import spark_bridge
            guard_eff = spark_bridge.load_guard_effectiveness()
            _spark_effective_guards = {
                g for g, delta in guard_eff.items() if delta > 0.03
            }
        except Exception:
            pass
        ...
        for agent in pop:
            ...
            # PT-validated agents get a gentle breeding nudge
            pt_bonus = 0.05 if agent.agent_id in self._pt_validated_ids else 0
            # Spark insight bonus: agents using Spark-validated guards get +0.03
            spark_bonus = 0.0
            if _spark_effective_guards:
                agent_guards = {
                    k for k in agent.mutations
                    if k.startswith("cr_") or k.endswith("_guard") or k.endswith("_filter")
                }
                if agent_guards & _spark_effective_guards:
                    spark_bonus = 0.03
            scores.append(
                (fitness_score + diversity_bonus + pt_bonus + spark_bonus) * children_penalty
            )
