...
# Add the original trading chip to path so we can import its backtest engine
...
def _load_guard_for_agent(mutations: dict[str, Any]) -> Callable | None:
    ...
    guard_id = mutations.get("llm_guard_id")
    if not guard_id:
        return None
    ...
    try:
        ...
        raw_fn = gen.load_guard(guard_id)
        if raw_fn is None:
            logger.debug("Guard %s not found on disk", guard_id)
            return None
        return _make_guard_adapter(raw_fn)
    except Exception as e:
        logger.warning("Failed to load guard %s: %s", guard_id, e)
        return None
...
def evaluate_agent(
    ...
    """
    root = runtime_root or TRADING_CHIP_ROOT
    ...
    # Load LLM guard if agent has one
    guard_fn = _load_guard_for_agent(mutations)
    if guard_fn:
        logger.debug("Applying guard %s during evaluation", mutations.get("llm_guard_id"))
    ...
    if result is None:
        return {
            ...
            "sharpe_ratio": 0, "trade_count": 0, "error": "no_data",
...
def staged_evaluate(
    ...
    Three-stage filtering saves ~3x compute by killing bad candidates early:
      Stage 1 (Quick):  2,000 contracts — reject if WR < 0.50 or trades < 5
